...
public class DTLSSocket {
    ...
    private static int MAXIMUM_PACKET_SIZE = 4*1024;
	
	public DTLSSocket(String protocolVersion, String sideType, String authType, String[] cipherSuites, String ksName,
			String ksPass, DatagramSocket sock, SocketAddress otherSideAddr) throws Exception {
		dSocket = sock;
	...
	private void setDTLSConfigurations(String protocolVersion, String authType,
			String[] cipherSuites) {
            ...
            if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP ||
                ...
                if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP) {
                    ...
                    try {
                        ...
                        System.out.println(selfType+":"+"TIMEOUT occured when expecting dtls record");
                        ...
                        if (finished) {
                            System.out.println(selfType + ":Handshake status is FINISHED after calling onReceiveTimeout()");
                            endLoops = true;
                        }
                ...
                if (hs == SSLEngineResult.HandshakeStatus.FINISHED) {
                    System.out.println(selfType+":Handshake status is FINISHED, finish the loop");
                    endLoops = true;
                }
                ...
                System.out.println(selfType+":Handshake status is NOT_HANDSHAKING, finish the loop");
                endLoops = true;
    ...
    public void send(ByteBuffer src) throws Exception {

        // Note: have not consider the packet loses
        List<DatagramPacket> packets =
                produceApplicationPackets(src);
        src.flip();
        for (DatagramPacket p : packets) {
            dSocket.send(p);
        }
	...
	boolean produceHandshakePackets(List<DatagramPacket> packets) throws Exception {
        ...
        while (!endLoops &&
            ...
            System.out.println(selfType + ": " + "Reproducing lost packets.. CURRENT HANDSHAKE STATUS is " + hs);
            if (rs == Status.BUFFER_OVERFLOW) {
            	
                System.out.println(selfType + ": " + "Buffer overflow- incorrect server maximum frag size");
            	throw new Exception("Buffer overflow: incorrect server maximum fragment size (SIDE:" +selfType+", HandshakeStats:"+ hs +")");
            
            } else if (rs == Status.BUFFER_UNDERFLOW) {
            	System.out.println(selfType + ": " + "Buffer overflow- incorrect server maximum frag size");
                if (hs != HandshakeStatus.NOT_HANDSHAKING) {
                	throw new Exception("Buffer underflow: incorrect server maximum fragment size (SIDE:" +selfType+", HandshakeStats:"+ hs +")");    
                } // otherwise, ignore this packet
            ...
            if (hs == SSLEngineResult.HandshakeStatus.FINISHED) {
            	System.out.println(selfType + ": " + "Reproducing lost packets.. HANDSHAKE STATUS is FINISHED");
                return true;
            }
            ...
            while (!endInnerLoop) {
                if (nhs == HandshakeStatus.NEED_TASK) {
                    runDelegatedTasks();
                } else if (nhs == HandshakeStatus.NEED_UNWRAP ||
                    nhs == HandshakeStatus.NEED_UNWRAP_AGAIN ||
